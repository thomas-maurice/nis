...
type UserRepository interface {
	...
	// rows. Used by P11 global search. Revoked users are included — the
	// caller can decide whether to hide them. Scope narrowing matches
	// ListPage (operator-admin sees users whose parent account belongs to
	// their operator; account-admin sees users in their account).
	Search(ctx context.Context, scope authz.Scope, q string, limit int) ([]*entities.User, error)
}
