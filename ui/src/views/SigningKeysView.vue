<template>
  <div class="container-fluid py-4">
    <div class="row g-2 mb-3 align-items-end">
      <div class="col-auto">
        <label for="ssk-account-filter" class="form-label small mb-1">Account</label>
        <select id="ssk-account-filter" v-model="accountFilter" class="form-select form-select-sm" @change="loadFirstPage">
          <option value="">All accounts</option>
          <option v-for="acc in accounts" :key="acc.id" :value="acc.id">{{ acc.name }}</option>
        </select>
      </div>
      <div class="col-auto">
        <label for="ssk-name-filter" class="form-label small mb-1">Name contains</label>
        <input id="ssk-name-filter" v-model="nameLikeInput" type="text" class="form-control form-control-sm" placeholder="substring..." @input="onNameLikeInput" />
      </div>
    </div>
...
const loadPage = async (cursor) => {
  ...
  try {
    ...
    if (accountFilter.value) {
      req.accountId = accountFilter.value
    }
    const response = await apiClient.post('/nis.v1.ScopedSigningKeyService/ListScopedSigningKeys', req)
